...
- `Sources/NexusApp/ShellWindowManager.swift` owns Nexus’s custom over-notch fullscreen mode, including saved windowed frame restoration, `NSApp.presentationOptions` changes, persistence keys per built-in display, and `ShellDisplayLayout` derivation from `NSScreen.safeAreaInsets` plus the auxiliary notch lobes. Inspect it first when over-notch mode restores the wrong frame, persists the wrong mode for a display, or mis-detects the camera housing geometry.
...
- `Sources/StageChrome/StageChromeView.swift` now has two shell-layout branches: the original windowed sidebar/topbar path and a notch-fill path that renders the stage viewport inside `ShellDisplayLayout.safeContentFrame` while moving shell chrome into compact leading/trailing top regions. Inspect it first when over-notch mode stages windows correctly but shell chrome lands in the wrong place, or when the reported stage viewport frame seems offset after entering custom fullscreen.
...
- `Sources/NexusApp/AppEnvironment.swift` and `Sources/WorkspaceEngine/WorkspaceSession.swift` now also own width reconciliation. Check them first when a visible app widens itself beyond the planned slot, when a focused native-window resize does not persist, or when width corrections appear to loop, because post-stage visible-slot observation, focused-window debounce, `SelectionOrigin.nativeGeometrySync`, and persisted `refreshSlotWidth(...)` writes all live in those two files rather than in `WindowChoreographyService`.
- `Sources/NexusApp/Resources/slot-presets.json`, `Sources/NexusApp/Resources/workspace-templates.json`, and `Sources/NexusApp/WorkspaceTemplateCatalog.swift` are now the source of truth for new-workspace starter layouts and per-bundle slot defaults. Check them first when the sidebar plus menu creates the wrong workspace shape, when starter labels or widths drift, or when Tether loses its hybrid preset, because `WorkspaceSession` no longer hardcodes any starter app mix.
- `Sources/NexusApp/FocusedWindowSlotFactory.swift` and `Sources/NexusApp/AppEnvironment.swift` now own manual add and auto-add capture. Inspect them first when "Add Focused Window" captures the wrong app, when auto-add produces duplicate or badly named slots, or when clicking the Nexus add button seems to ignore the last external app, because manual add now reuses the last externally focused candidate cached by `AppEnvironment` instead of querying the currently frontmost window after Nexus steals focus.
- `Sources/WorkspaceEngine/WorkspaceSession.swift` now also owns slot CRUD, slot reordering, and selected-workspace auto-add policy persistence. Check it first when move-left or move-right order is not saved, when removing a slot selects the wrong neighbor, or when the `Auto-add` chip/menu state flips but does not survive relaunch.
...
- `Sources/StageChrome/ChromeTheme.swift` uses `ChromeMetrics.stageGeometry(for:shellPresentationMode:)` to switch between the old sidebar/topbar chrome tax in windowed mode and a zero-sidebar/zero-topbar safe-content viewport in over-notch mode. Check it first when fullscreen-over-notch unexpectedly leaves space unused or when windowed layout regresses after shell-presentation changes.
